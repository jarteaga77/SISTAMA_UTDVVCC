 ...
 * @author jarteaga
 */
public class DaoOrdenCompraCSS 

{
    ...
    public static void setId_orden(long id_orden) {
        DaoOrdenCompraCSS.id_orden = id_orden;
    }
    

    public DaoOrdenCompraCSS() {
    }
 
    public void guardarOrden(OrdenCompraCSS orden)
    {
        try
        {
           ResultSet obj= nueva.executeQuery("INSERT INTO ordenescompra.orden_compra_css(fecha_transaccion,nit_proveedor,id_itemccostos,requisicion,cotizacion,cargoa_orden,dto_financiero,observacion,id_usuario,condiciones)VALUES"
    ...
    public void actualizarOrden(OrdenCompraCSS orden)
    {
        try
        {
            nueva.executeUpdateQuery("UPDATE ordenescompra.orden_compra_css SET nit_proveedor="+orden.getNit_proveedor()+",fecha_transaccion='"+orden.getFecha_hora()+"', cotizacion='"+orden.getCotizacion()+"', id_itemccostos="+orden.getId_item_ccostos()+", cargoa_orden='"+orden.getCargo_orden()+"', dto_financiero="+orden.getDto_financiero()+", observacion='"+orden.getObservacion()+"', requisicion='"+orden.getRequisicion()+"', condiciones='"+orden.getCondiciones()+"' WHERE consecutivo_orden_compra="+orden.getConsecutivo()+"");
            nueva.desconectar();            

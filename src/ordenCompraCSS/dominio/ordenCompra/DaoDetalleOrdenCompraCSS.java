...

{
    ...
    public void updateDetalle(Detalle_orden_compraCSS detalle, int id_detalle)
    {
         try
        {
       ...
       // long subtotal=(long) (detalle.getVlorUnitario() * detalle.getCantidad());
        ...
        nueva.executeUpdateQuery("UPDATE ordenescompra.detalle_orden_compra_css SET descripcion='"+detalle.getDescripcion()+"', cantidad="+detalle.getCantidad()+", referencia='"+detalle.getReferencia()+"', vlor_unitario="+detalle.getVlorUnitario()+", subtotal="+detalle.getSubtotal()+", id_unidad="+detalle.getId_unidad()+", total="+detalle.getTotal()+" WHERE id_detalle_orden_compra="+id_detalle+" AND consecutivo_ordencompra="+detalle.getId_consecutivo_orden()+"");
        nueva.desconectar();
